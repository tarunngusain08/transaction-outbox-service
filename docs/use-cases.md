...
| UC-09 | Implemented | Client calls a retired V1 create, normalize, or GET-by-ID route | Returns `410 Gone`, a stable problem type, and the exact V2 collection, normalize, or same raw ID-segment successor without binding a V1 body or parsing the retired ID as UUID | Valid and malformed ID segments create no transaction/outbox row, and V1 is never silently interpreted as V2 |
| UC-10 | Implemented | Deployment operator stops writers, snapshots the database, and runs the read-only V7 preflight before Flyway | Exact canonical V1-V6 history plus a zero historical-unpublished count returns success and permits the V6-to-V7 upgrade; V7 installs quarantine evidence/constraints/index without rewriting V1-V6 | Missing/changed/extra/already-applied migration state or any unresolved count exits nonzero; emergency V7 containment can quarantine rows but does not complete historical delivery, and UC-P08 is still required |
...
| ENG-07 | `make migration-preflight`, `make migration-v7-preflight` | Read-only repeatable-read reports expose identity/currency collisions and historical unpublished events; the V7 command itself rejects noncanonical Flyway state and nonzero unresolved counts, while operators retain both worklists as evidence |
